<?php
    ...
    // Get variable from request
        (isset($_GET['utilType'])) ? $utilType = $_GET['utilType'] : null;
    ...
    //ชื่อตาราง
    $table = 'egatpea_outage';//'fdr_outage_view';//
    //ชื่อคีย์หลัก
    $primaryKey =  'egatpea_outage_id';//'num';//
    //ข้อมูลอะเรที่ส่งป datables
    $columns = array(
        array(  'db' => 'event_date', 'dt' => 0),
        array(  'db' => 'utility', 'dt' => 1 ),
        ...
        array(  'db' => 'lateral', 'dt' => 8),
        array(  'db' => 'affected_fdrs', 'dt' => 9),
        array(  'db' => 'fdr_minutes', 'dt' => 10),
    );
    //เชื่อต่อฐานข้อมูล
    $sql_details = array(
    'user' => 'root',
    ...
    $whereAll = "";
    if (isset($utilType)) {
        $whereAll = ($utilType !== 'all') ? "utility LIKE '$utilType%' AND " : "";
    } else {
