$(function () {
    ...
    displayHilightMenu('search-feeder-index');
    ...
    $('[type="submit"]').click((e) => {
      ...
      } else if (selectedForm[0].attr('id') === 'district') {
        //Collapse search input card
        $('[data-card-widget="collapse"').click();
        if (typeof table === 'undefined') {
          createDataTable();
    ...
    function updateCaption({numRows}) {
      ...
      if (selectedForm[0].attr('id') === 'feeder') {
        ...
        if (numRows) {
          $('caption').text(`สถิติไฟฟ้าดับการไฟฟ้านครหลวงเขต ${selectedDist} วันที่ ${moment(dateFrom).format('ll')} ถึงวันที่ ${moment(dateTo).format('ll')}`);
        } else {
          $('caption').text(`ไม่พบ สถิติไฟฟ้าดับการไฟฟ้านครหลวงเขต ${selectedDist} วันที่ ${moment(dateFrom).format('ll')} ถึงวันที่ ${moment(dateTo).format('ll')}`);
        }
      } else { // id='cause' 
    ...
    function getFormValue() {
      ...
      if (selectedForm[0].attr('id') === 'feeder') {
        ...
        return {selectedDistValue, selectedDist, outageSystem, intType, dateFrom, dateTo};
      } else { // id='cause'
        let selectedCauses = $('[name="selectedCauses[]"]').select2('data');
        let selectedCause = selectedCauses.map(selectedCause => selectedCause.id);
        return {selectedCause, outageSystem, intType, dateFrom, dateTo};
      }
    }
    // /.Utility function
});