$(function () {
  ...
  displayHilightMenu('search-station-index');
  ...
  $('[name="stationName"]').bind('keydown', function(e) { //on keydown for all textboxes  
  ...
  $.get('./api/search-station-outage-date-range-api.php', function(res) {
    moment.locale('th');
  ...
  $('[type="submit"]').click((e) => {
    ...
    if (selectedForm[0].attr('id') === 'station') {
      if (validateStation({selectedInputElem : selectedForm[0].find('[name="stationName"]'), selectedCheckElem : selectedForm[0].find('#allStation')})) {
    ...
    function validateStation({selectedInputElem, selectedCheckElem}) {
      const validateValue = selectedInputElem.val().trim();
      const validateAllStation = selectedCheckElem.is(':checked');
      if (validateValue === "" && !validateAllStation) {
      ...
      if (validateValue !== "" && !validateAllStation) {
        if (!( (/^[a-zA-Z]{2,3}$/.test(validateValue)) || (/(^[a-zA-Z]{2} [0-9]{1,5}$)/.test(validateValue)) || (/(^[a-zA-Z]{3}[0-9]{1,5}$)/.test(validateValue)))) {
      ...
      } else { // validateAllStation (allStation is chedcked)
  ...
  function createDataTable() {
    jQuery.fn.DataTable.Api.register( 'buttons.exportData()', function ( options ) {
      if ( this.context.length ) {
          let jsonResult = $.ajax({
              url: './api/search-station-index-api.php',
              data: getFormValue(),
    ...
    table = $("#outageTable").DataTable({ // table as global variable
      ...
      ajax: {
        ...
        type: "GET",
        data: function(d) {
          let {stationName, selectedCause, intType, dateFrom, dateTo} = getFormValue();
          (typeof stationName !== 'undefined') ? d.stationName = stationName : null;
  ...
  function updateCaption({numRows}) {
    let selectedForm = activeFormElems.filter(activeFormElem => activeFormElem.attr('class').includes('active'))
    if (selectedForm[0].attr('id') === 'station') {
      let {stationName, dateFrom, dateTo} = getFormValue();
      stationName = (stationName === 'allStation') ? 'ทุกสถานีฯ' : stationName;
      if (numRows) {
        $('caption').text(`สถิติไฟฟ้าดับสถานีฯ ${stationName} วันที่ ${moment(dateFrom).format('ll')} ถึงวันที่ ${moment(dateTo).format('ll')}`);
      } else {
        $('caption').text(`ไม่พบ สถิติไฟฟ้าดับสถานีฯ ${stationName} วันที่ ${moment(dateFrom).format('ll')} ถึงวันที่ ${moment(dateTo).format('ll')}`);
      }
    } else { // id='cause' 
  ...
  function getFormValue() {
    ...
    if (selectedForm[0].attr('id') === 'station') {
      let stationName;
      if (selectedForm[0].find('#allStation').is(':checked')) { // allStation is checked
        stationName = 'allStation';
      } else { // allStation isn't checked but input has station name
        stationName = $("input[name='stationName']").val().substr(0,3);
      }
      return {stationName, intType, dateFrom, dateTo};
    } else { // id='cause'
