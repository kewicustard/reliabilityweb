$(function () {
    ...
    displayHilightMenu('search-egatpea-index');
    ...
    $.get('./api/search-egatpea-outage-date-range-api.php', function(res) {
      moment.locale('th');
    ...
    $('[type="submit"]').click((e) => {
      ...
      if (selectedForm[0].attr('id') === 'utilities') {
    ...
    function createDataTable() {
      jQuery.fn.DataTable.Api.register( 'buttons.exportData()', function ( options ) {
        if ( this.context.length ) {
            let jsonResult = $.ajax({
                url: './api/search-egatpea-index-api.php',
                data: getFormValue(),
      ...
      table = $("#outageTable").DataTable({ // table as global variable
        ...
        ajax: {
          ...
          type: "GET",
          data: function(d) {
            let {utilType, selectedCause, intType, dateFrom, dateTo} = getFormValue();
            (typeof utilType !== 'undefined') ? d.utilType = utilType : null;
    ...
    function updateCaption({numRows}) {
      let selectedForm = activeFormElems.filter(activeFormElem => activeFormElem.attr('class').includes('active'))
      if (selectedForm[0].attr('id') === 'utilities') {
        let {utilType, dateFrom, dateTo} = getFormValue();
        if (numRows) {
          $('caption').text(`สถิติไฟฟ้าดับจาก ${utilType} วันที่ ${moment(dateFrom).format('ll')} ถึงวันที่ ${moment(dateTo).format('ll')}`);
        } else {
          $('caption').text(`ไม่พบ สถิติไฟฟ้าดับจาก ${utilType} วันที่ ${moment(dateFrom).format('ll')} ถึงวันที่ ${moment(dateTo).format('ll')}`);
        }
      } else { // id='cause' 
    ...
    function getFormValue() {
      ...
      let selectedForm = activeFormElems.filter(activeFormElem => activeFormElem.attr('class').includes('active'))
      if (selectedForm[0].attr('id') === 'utilities') {
        let utilType = utilitiesType[parseInt($("input[name='utilities']:checked").val())];
        return {utilType, intType, dateFrom, dateTo};
      } else { // id='cause'
